...
##🤝 遇到的问题 (Known Issues)
- **关于 LLM 上下文中断的问题**
目前插件会将执行结果直接发送给用户，并强制结束工具调用，而不是返回给 LLM。
为什么这样做？
我也想把结果扔回给 LLM 让它润色一下，但是这会导致死循环：
LLM 调用工具 -> 2. 获得结果 -> 3. LLM 思考如何回答 -> 4. LLM 觉得还需要再算一遍 -> 5. 再次调用工具...
为了防止你的额度和 Token 爆炸，我选择了最稳妥的方式：算完直接发出来，顺手把 LLM 的嘴堵上。 🤐
所以你家bot事实上并不知道自己干了什么
## 🍪 
虽然现在只能跑代码，但我还有很多想法：
