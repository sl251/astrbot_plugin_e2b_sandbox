# AstrBot E2B 云沙箱插件 (E2B Sandbox)

使用 E2B 云沙箱安全执行 Python 代码，支持通过 LLM 自然语言调用。

这是 AstrBot 的一个增强插件，它替换了本地不安全的 Python 执行环境，将代码运行托管在隔离的云端沙箱中，支持访问网络、安装第三方库，且无需担心破坏本地宿主机环境。
...
1.  **💻 环境兼容性警告**：
    **本插件目前仅在 Windows 本地部署环境下进行过测试和验证。**
...
<img width="317" height="182" alt="image" src="https://github.com/user-attachments/assets/3bb8999b-fcfb-4400-80dd-923702f1f337" />
...
- **实际算账**：
    - $100 额度 ≈ **2000 小时** 的运行时间。
    - 每次代码执行通常只需几秒钟，执行完沙箱即刻销毁。
    - 假设你每天让机器人跑 100 次代码，每天总消耗可能不到 0.05 美元。
    - **结论**：对于个人聊天机器人用途，这 $100 额度用个几年都没问题。
...
- "帮我计算 1 到 100 的质数之和"
- "用 Python 获取当前 Google 首页的标题"
- "执行代码: print('Hello E2B')"

**运行示例：**

```text
[INFO] astrbot/astrbot: 执行代码 print('hello E2B')
[INFO] Agent 使用工具: ['run_python_code']
[INFO] [E2B] 开始执行代码: print('hello E2B')
[INFO] [E2B] 正在创建沙箱...
[INFO] [E2B] 沙箱创建成功，开始执行代码...
[INFO] [E2B] 返回结果: 输出: hello E2B
[INFO] [E2B] 沙箱已关闭
[INFO] astrbot/astrbot: 代码已成功执行，结果如下：输出: hello E2B
```
